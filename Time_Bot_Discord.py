...
class Timing(HighClass):
    ...
    async def checktime(self, ctx, member: discord.Option(discord.Member, "Enter someone", required=True)):
        ...
        if features["Track Time"]:
            ...
            await ctx.respond("Your admins have turned off this feature.")
    ...
    @bridge.bridge_command(name='starttime', description='Check when the bot started')
...
def run():
    ...
    act_list_copy = act_list.copy()
    for member_ in act_list_copy.keys():
        act_list[member_][1] = False
    while running:
        for member_ in act_list_copy.keys():
            if not member_.status == discord.Status.offline:
                act_list_copy[member_][3][-1] = datetime.now() - act_list_copy[member_][0]
                act_list_copy[member_][1] = True
            elif member_.status == discord.Status.offline:
                act_list_copy[member_][0] = datetime.now()
                if act_list_copy[member_][1]:
                    act_list_copy[member_][1] = False
                    act_list_copy[member_][3].append(timedelta(seconds=0))
                    if len(act_list_copy[member_][3]) > 300:
                        act_list_copy[member_][3][0] = sum(act_list_copy[member_][3])

class Fun(HighClass):
    @bridge.bridge_command(name='rate', description='Rates you!')
